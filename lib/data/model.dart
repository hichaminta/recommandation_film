...
class MovieModel {
  ...
  factory MovieModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json.containsKey('genre_ids')) {
      genresList = (json['genre_ids'] as List)
...
Future<List<MovieModel>> fetchLikedMovies() async {
  ...
  for (String id in likedIds) {
    final response = await http.get(
      Uri.parse('https://api.themoviedb.org/3/movie/$id?language=fr-FR'),
      headers: {
        'Authorization': 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1NjZhN2M4MjcxZDU3N2JjNzdmMmU1ZDgwY2U0NjlkMCIsIm5iZiI6MTc0NDY3MTMxNS4yNDk5OTk4LCJzdWIiOiI2N2ZkOTI1MzYxYjFjNGJiMzI5OTRmNDciLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.fKG7SaYBn-FtJeMEuZJ6c8D4VjQxYvUJ9ngVxln0zns',
        'Accept': 'application/json',
      },
...
Future<List<MovieModel>> fetchRecommendationsFromLikedMovies() async {
  final prefs = await SharedPreferences.getInstance();
  final likedIds = prefs.getStringList('likedMovies') ?? []; // Récupérer la liste des IDs des films aimés

  Set<String> seenMovieIds = {}; // Pour éviter les doublons
  List<MovieModel> recommendations = [];

  for (String id in likedIds) {
    final response = await http.get(
      Uri.parse('https://api.themoviedb.org/3/movie/$id/recommendations?language=fr-FR'),
      headers: {
        'Authorization': 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1NjZhN2M4MjcxZDU3N2JjNzdmMmU1ZDgwY2U0NjlkMCIsIm5iZiI6MTc0NDY3MTMxNS4yNDk5OTk4LCJzdWIiOiI2N2ZkOTI1MzYxYjFjNGJiMzI5OTRmNDciLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.fKG7SaYBn-FtJeMEuZJ6c8D4VjQxYvUJ9ngVxln0zns',
        'Accept': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      ...
      for (var data in jsonData) {
        ...
        if (!seenMovieIds.contains(movieId)) {
          ...
          recommendations.add(MovieModel.fromJson(data)); // Ajouter le film recommandé
        }
      }
    } else {
      print('Erreur lors de la récupération des recommandations pour le film $id');
    }
  }

  // Retourner la liste complète des films recommandés
  return recommendations;
}
Future<List<MovieModel>> fetchAllMovies({int maxPages = 10}) async {
  const bearerToken = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI1NjZhN2M4MjcxZDU3N2JjNzdmMmU1ZDgwY2U0NjlkMCIsIm5iZiI6MTc0NDY3MTMxNS4yNDk5OTk4LCJzdWIiOiI2N2ZkOTI1MzYxYjFjNGJiMzI5OTRmNDciLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.fKG7SaYBn-FtJeMEuZJ6c8D4VjQxYvUJ9ngVxln0zns';
